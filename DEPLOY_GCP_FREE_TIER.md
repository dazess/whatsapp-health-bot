...
After creating the VM and opening ports 22/80/443, SSH into the VM and run this block.
...
git clone YOUR_GIT_REPO_URL whatsapp-health-bot
...
cp .env.example .env
echo "Now edit .env with real secrets and OAuth values"
nano .env

./deploy/start_pm2.sh
pm2 status

echo "Open QR logs and scan once from WhatsApp"
pm2 logs healthbot-wa-service --lines 200

echo "After your domain points to this VM, enable HTTPS"
